      ...
      public void beforeTextChanged(CharSequence s, int start, int count, int after) {
        if(!received && count > 0) {
          if(fixThis.length() != 0) {
            ...
            event.setId(userId);
            event.setCursorpos(docText.getSelectionStart());
            event.setBeforetext("");
      ...
      public void onTextChanged(CharSequence s, int start, int before, int count)
      {
        if(!received && count > 0) {
          if(removeText.length() != 0) {
            Event.Builder event = Event.newBuilder();
            event.setId(userId);
            event.setCursorpos(docText.getSelectionStart()-count);
...
/////////////////////////////////////////////////////////////////////////           
            ...
            if(eventRecvd.getId() != userId) {
              ...
              if(after.length() > 0) {
                //INSERT
                received = true;
                docText.getText().replace(cursor-after.length(), cursor-after.length(), after);
              }
              else {
                //REMOVE
                received = true;
                docText.getText().replace(cursor, cursor+before.length(), after);
              }        
            }
            //RIGHT FREAKIN HERE
          }
        ...
        /*
          ...
          builder.setPositiveButton("OK", new DialogInterface.OnClickListener() { 
              ...
              public void onClick(DialogInterface dialog, int which) {
                  ...
                  if(sessionNames.contains(m_Text)) {
                    try
                    {
                      int index = -1;
